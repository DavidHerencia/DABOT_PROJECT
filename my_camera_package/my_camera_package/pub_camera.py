...
class ImagePublisher(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Image, 'camera/image_raw', 15)
        self.timer = self.create_timer(0.15, self.timer_callback)  # Publica cada 0.1s (10 Hz)
        ...
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # Ancho de la imagen
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)  # Alto de la imagen
        ...
        if (self.cap.get(cv2.CAP_PROP_FOURCC) != cv2.VideoWriter_fourcc(*'MJPG') or
                self.cap.get(cv2.CAP_PROP_FRAME_WIDTH) != 1280 or
                self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT) != 720):
